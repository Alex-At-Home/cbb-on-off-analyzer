...
const PlayerLeaderboardTable: React.FunctionComponent<Props> = ({
  ...
  const buildFilterStringTest = (player: IndivStatSet) => {
    ...
    const usefulFormatBuilder = (s: string) => {
      return `${player.roster?.year_class || ""}_${s || ""}:${player.team || ""
        }_${player.year || ""}`;
    };
    return (
  ...
  const { table, maybeMap } = React.useMemo(() => {
    ...
    const builderPlayerLine = (
      ...
      const rosterInfoText = `${height && height != "-" ? height : ""} ${yearClass ? yearClass : ""
        }${rosterNum ? ` / #${rosterNum}` : ""}`;
    ...
    setNumFilteredStr(
      isFiltered
        ? `, filtered: ${tier == "All" && numFiltered > parseInt(maxTableSize)
      ...
      >
        {dataEvent.syntheticData ? null : (
          <Form.Group as={Row}>
            ...
            >
              <ConferenceSelector
                emptyLabel={`All ${!_.isEmpty(transferInfoSplit[0]) ? "Transfers" : "Teams"

...
export class PlayerSimilarityUtils {
  ...
  // (this replaces the dynamicRangeWeights, so we can remove this - except for lowFreqStylesWeight/medFreqStylesWeight used in buildSimplePlayerSimilarityVector)
  ...
  static readonly buildPlayerSimilarityVector = (
    ...
    const additionalPlayStyleStats = [
      {
        value: player.off_assist?.value ?? 0,
        weight: config.assistWeighting,
        dynamicWeight: PlayerSimilarityUtils.dynamicRangeWeights.assists,
      },
      {
        value: player.off_to?.value ?? 0,
        weight: config.turnoverWeighting,
        dynamicWeight: PlayerSimilarityUtils.dynamicRangeWeights.turnovers,
      },
      {
        value: player.off_orb?.value ?? 0,
        weight: config.offensiveReboundWeighting,
        dynamicWeight: PlayerSimilarityUtils.dynamicRangeWeights.offRebounds,
      },
      {
        value: player.off_ftr?.value ?? 0,
        weight: config.freeThrowWeighting,
        dynamicWeight: PlayerSimilarityUtils.dynamicRangeWeights.freeThrows,
      },
    ];
    ...
    if (config.scoringMode === "relative") {
      ...
      weightedAveragePts =
        totalWeight > 0 ? weightedAveragePts / totalWeight : 1.0;
    }
    ...
    if (config.fgBonus !== "none") {
      const fgStats = [
        ...
        {
          pct: player.off_2pmid?.value ?? 0,
          rate: player.off_2pmidr?.value ?? 0,
        },
        {
          pct: player.off_2prim?.value ?? 0,
          rate: player.off_2primr?.value ?? 0,
        },
      ];
    ...
    if (config.defensiveSkill !== "none") {
      ...
      switch (config.defensiveSkill) {
        ...
        case "raw":
          ...
          defensiveSkill =
            -(player.def_rtg?.value ?? avgEfficiency) + avgEfficiency;
          break;
        case "relative":
          const defRapm = (player.def_adj_rapm as Statistic)?.value ?? 0;
      ...
      if (includeWeights) {
        if (config.defensiveSkill === "raw") {
          ...
          defensiveSkill *=
            PlayerSimilarityUtils.dynamicRangeWeights.perPossession;
        } else {
      ...
      if (includeWeights) {
        ...
        drb *=
          PlayerSimilarityUtils.dropdownWeights[
            config.defensiveReboundWeighting
          ];
    ...
    if (config.classWeighting !== "none") {
      let playerClass = PlayerSimilarityUtils.parsePlayerClass(
        player.roster?.year_class
      );
      ...
      if (includeWeights) {
        ...
        playerClass *=
          PlayerSimilarityUtils.dropdownWeights[config.classWeighting];
    ...
    if (config.minutesWeighting !== "none") {
      ...
      if (includeWeights) {
        ...
        minutes *=
          PlayerSimilarityUtils.dropdownWeights[config.minutesWeighting];

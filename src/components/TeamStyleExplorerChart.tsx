...
const TeamStyleExplorerChart: React.FunctionComponent<Props> = ({
  startingState,
  dataEvent,
  onChangeState,
}) => {
  ...
  const chart = React.useMemo(() => {
    ...
    const medFreqWeight = new Set<TopLevelPlayType>([
      "Put-Back",
      "Post-Up",
      "Big Cut & Roll",
      "Mid-Range",
    ...
    const playFreqWeight = 3.0; //(vs shot making)
    const pppNormalization = 0.2; // so 1ppp == 20% in weight

    const teamsPhase1 = phase1Processing(dataEvent.teams || []);
    if (_.isEmpty(teamsPhase1)) return;

    const vectors = _.map(teamsPhase1, (team) => {
      // Additional offensive metrics: none?
      ...
      const extraOff = [0.25 * (team.off_ftr?.value || 0)];

      // Additional defensive metrics: STL%, non-STL TO%, block, FTR

      const defStyle = team.def_style as TopLevelPlayAnalysis;
      const defLevel = 0.01 * (team.def_ppp?.value || 100) * 0.9;

      const defs = _.flatMap(PlayTypeUtils.topLevelPlayTypes, (playType) => {
        const def = defStyle[playType];
        const weight = lowFreqWeight.has(playType)
          ? 3.0
          : medFreqWeight.has(playType)
          ? 1.5
          : 1;
        return [
          (pppNormalization * (def.pts?.value || 0)) / defLevel,
          (def.possPct?.value || 0) * playFreqWeight,
        ];
      });

      // weight down blocks and esp FTR (since it's a dup value)
      const extraDef = [
        team.def_stl?.value || 0,
        team.def_to_nonstl?.value || 0,
    ...
    /**/
    //IN
    console.log(_.take(vectors, 10));

    const nEpochs = umap.initializeFit(vectors);
    for (let i = 0; i < nEpochs; i++) {
      /**/
      if (i % 50 == 0) console.log(`${i} of ${nEpochs}`);
      umap.step();
    }
...
export default TeamStyleExplorerChart;

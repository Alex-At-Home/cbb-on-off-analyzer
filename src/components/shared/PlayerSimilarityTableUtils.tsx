...
const styleGroups = {
  "On-Ball": [
    "Rim Attack", "Attack & Kick", "Dribble Jumper", "Hits Cutter", 
    "PnR Passer", "Post-Up", "Post & Kick"
  ],
  "Off-Ball": [
    "Perimeter Sniper", "Mid-Range", "Backdoor Cut", "Big Cut & Roll",
    "Pick & Pop", "High-Low", "Put-Back"
  ],
...
export class PlayerSimilarityTableUtils {
  ...
  private static getZScoreColor = (zScore: number): string => {
    const percentage = PlayerSimilarityTableUtils.zScoreToPercentage(zScore) / 100;
                  ...
                  <div key={component.name} className="col-3">
                    ...
                    <ul className="list-unstyled mb-0" style={{ fontSize: '0.8rem' }}>
                      {component.breakdown
                        ...
                        .map((stat) => {
                          const percentage = PlayerSimilarityTableUtils.zScoreToPercentage(stat.zScore);
                          const color = PlayerSimilarityTableUtils.getZScoreColor(stat.zScore);
  ...
  private static buildSimpleStatBreakdownTooltip = (breakdown: StatBreakdown[]): React.ReactElement => {
    const { good, bad } = PlayerSimilarityTableUtils.categorizeStats(breakdown);

    return (
      <div style={{ maxWidth: '280px', backgroundColor: '#2c3e50', color: 'white', padding: '8px' }}>
        <div className="font-weight-bold mb-2">Component Breakdown</div>
        
        {good.length > 0 && (
          <div className="mb-2">
            <div style={{ color: CbbColors.getRedToGreen().domain([0, 1])(1).toString() }} className="font-weight-bold">
              Good Matches:
            </div>
            {good.map(stat => {
              const percentage = PlayerSimilarityTableUtils.zScoreToPercentage(stat.zScore);
              return (
                <div key={stat.name} className="d-flex justify-content-between">
                  <span>{stat.name}:</span>
                  <span style={{ color: PlayerSimilarityTableUtils.getZScoreColor(stat.zScore) }}>
                    {Math.round(percentage)}%
                  </span>
                </div>
              );
        ...
        {bad.length > 0 && (
          <div>
            <div style={{ color: CbbColors.getRedToGreen().domain([0, 1])(0).toString() }} className="font-weight-bold">
              Poor Matches:
            </div>
            {bad.map(stat => {
              const percentage = PlayerSimilarityTableUtils.zScoreToPercentage(stat.zScore);
        ...
        {good.length === 0 && bad.length === 0 && (
          <div className="text-muted">All matches are moderate (0.75 ≤ |z| ≤ 1.5)</div>
        )}
      </div>
    );
  };

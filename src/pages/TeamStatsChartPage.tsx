...
import TeamStatsExplorerChart from "../components/TeamStatsExplorerChart";

type Props = {
  testMode?: boolean; //works around SSR issues, see below
};
const TeamStatsChartPage: NextPage<Props> = ({ testMode }) => {
  ...
  function getRootUrl(params: TeamStatsExplorerChartParams) {
    return UrlRouting.TeamStatsExplorerChart(params);
  }

  const [teamStatsExplorerParams, setTeamStatsExplorerParams] = useState(
    UrlRouting.removedSavedKeys(allParams) as TeamStatsExplorerChartParams
  );
  const teamStatsExplorerParamsRef = useRef<TeamStatsExplorerChartParams>();
  teamStatsExplorerParamsRef.current = teamStatsExplorerParams;

  const teamStatsExplorerParamsChange = (
    rawParams: TeamStatsExplorerChartParams
  ) => {
    const params = _.omit(
      rawParams,
      _.flatten([
    ...
    <Container className="medium_screen">
      ...
      <Row className="mt-2">
        <Col xs={12} className="text-center">
          <LandingPageIcon />
          <h3>Team Stats Explorer Charts</h3>
        </Col>
      </Row>
...
export default TeamStatsChartPage;
